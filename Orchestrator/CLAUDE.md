...
| `test_engine.py` | API pubblica + stato globale + batch asincroni |
| `test_phases.py` | Esecutori di fase (snapshot/patch/reboot/validate/services/rollback) + retry |
| `test_db.py` | Helper DB puri per test engine (nessuno stato globale) |
...
# Opzionali (hanno default ragionevoli, non serve modificarli salvo infrastruttura non standard)
NODE_EXPORTER_PORT=9100        # porta node_exporter sui sistemi test
PROMETHEUS_RATE_WINDOW=5m      # finestra lookback query rate()
```
...
- `_NODE_EXPORTER_PORT` e `_RATE_WINDOW` in `prometheus_client.py` letti da `Config` (configurabili via `.env`: `NODE_EXPORTER_PORT`, `PROMETHEUS_RATE_WINDOW`)
...
- **Refactoring test_engine** (v1.4.0): `test_engine.py` (1200+ righe) → 3 moduli:
  - `test_db.py` — helper DB puri (pick_next_queued, create/update fasi, batch DB)
  - `test_phases.py` — fase executors + classify_error + maybe_retry + execute_test
  - `test_engine.py` — stato globale + API pubblica + batch asincroni (slim)
- **DRY api_client.py**: 4 wrapper HTTP identici → unico `_request()` + thin wrappers
- **Test suite completa** (`tests/`): 276 test, 0 fail — puri (serializers, poller, queue_manager, uyuni_client) + mock Flask (health, notification_manager, sync, queue, tests, approvals)
