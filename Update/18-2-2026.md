...
**mgrctl** è un wrapper semplificato attorno a podman exec specifico per il container UYUNI. Fa parte del pacchetto mgradm (lo stesso usato per l'installazione con mgradm install podman).
## Proxy requisiti e constraint bug 
...
**Taskomatic** è il **servizio di scheduling e orchestrazione dei job backend** di Uyuni (ereditato da Spacewalk).  In termini pratici è il **motore che esegue le operazioni pianificate** sul server